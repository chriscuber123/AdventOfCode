...
def part2(s):
    ...
    idents = []
    priority = 0
    for i in range(0, len(sLines), 3):
        r1, r2, r3 = sLines[i], sLines[i+1], sLines[i+2]
        itemType = set(r1).intersection(set(r2).intersection(set(r3)))
        idents.append(list(itemType)[0])
    for c in idents:
        if c.islower():
            priority += ord(c) - 96
        else:
...
i = '''DsPhSBQQQhqmBDhPDsFwjwsLjlRjlttvjvvtRb
